...
author: "Irene Garicia"
date: "November 28t 2021"
output: 
  html_document:
...
We are trying to find the best two variables to predict the weight of each type of fish. 
...
Obtain subset of fish with the species we want to analyze, then we will look at how well each combination of the predictor variables can predict the weight of the fish. We are using the perch subset in this analysis. 
...
##Now asdd in scatterplot

```{r}
basicNN + geom_point()
```

The scatter plot shows that as the Length1 of a perch goes up, its length also goes up. This means that length1 is a good determining factor for weight, and vice versa, wight is a good determining factor for predicting Length1.


```{r}
basicNN + geom_point() + geom_smooth(method=lm)
```
Both graphs shows that as legth1 increases, weight follows and increases as well. It looks like a straight line is not a great fit because most of the points dont fall evenly around the blue line. We still need more tests to find the best two variables. 
...
```
The graph shows that the Y axis is adjusted R squared, and the X axis is the number of variables used in a linear fit. The graph shows that width; subset 2 is the best fit.
...
$H_A$: there is a difference between models 1 and 2 when predicting the weight of fish (perch)

```{r}
anova(fullup,best.model)
```

The analysis of this table shows that model 1 is slightly better because it has a smaller residual sum of squares.

# Discussion and Conclusion

<!--Discussion and conclusion here.  If you found a relationship be sure to consider whether the relationship occurs because one of the variavbles causes the other, or whether they perhasps are related for some other reason.  Watch the chapter 6 videos from the GeorgeTown videos collection.!-->

We fail to reject the null hypothesis because the P value is hight at 0.879. We conclude that height and width are the two best variables when predicting weight if using just two variables.
...
The scatterplot shows that as weight of roach goes up, legth also goes up. Weigh is a good determining fatctor.
```{r}
basicNN + geom_point() + geom_smooth(method=lm)
```
Both graphs show that as legth increases weight follows and increases. It looks like a straight line is not a great fit because most of the points dont fall evenly around blue line. We need more test to find the best two variables.
...
The graph shows that the Y axis is adjusted R squared, and the X axis is the number of variables used in a linear fit. The graph shows that the best fit comes from using two variables Length3 and width; subset 2 is the best fit. 
...
summary.out$which[2,]
```
The summary shows which 2 variables are best for a determining the weight of roach which is length and width. 
